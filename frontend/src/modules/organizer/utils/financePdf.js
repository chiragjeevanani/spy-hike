...
export function downloadFinancialReportPDF({ organizer, bookings, payouts }) {
  ...
  const drawContinuationHeader = () => {
    text(M, 36, 'FINDYOURTREK', { font: 'F2', size: 13, color: BROWN });
  ...
  pagesOps.forEach((page, i) => {
    const footerStr = `Page ${i + 1} of ${totalPages}  |  Find Your Trek Partner Network - Confidential Financial Report`;
  ...
  a.download = `Find Your Trek-Financial-Report-${(organizer?.agencyName || organizer?.name || 'Partner').replace(/[^a-zA-Z0-9]+/g, '-')}-${stamp}.pdf`;
