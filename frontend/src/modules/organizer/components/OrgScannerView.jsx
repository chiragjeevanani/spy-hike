...
import {
  ...
  Ticket, AlertTriangle, Hash, RefreshCw, Clock, ShieldAlert
} from 'lucide-react';
  ...
  return (
    <motion.div
      ...
      className={`w-full rounded-3xl overflow-hidden border transition-colors ${
        darkMode ? 'bg-zinc-900/90 border-zinc-800 text-white shadow-xl' : 'bg-white border-zinc-200/90 text-zinc-900 shadow-md'
      }`}
    >
      {/* Card header */}
      <div className={`px-5 pt-4 sm:pt-5 pb-3.5 border-b ${
        darkMode ? 'bg-zinc-800/60 border-white/5' : 'bg-gradient-to-br from-orange-50/80 to-amber-50/50 border-zinc-100'
      }`}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <h2 className={`text-base font-display font-black leading-tight line-clamp-2 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>
              {booking.tripName || 'Trek'}
            </h2>
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              <StatusBadge status={booking.status || 'Upcoming'} />
              <span className={`text-[10px] font-mono ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>{booking.bookingId || booking.id}</span>
            </div>
          </div>
        ...
        {rows.map(({ icon: Icon, label, value }) =>
          value && value !== '—' ? (
            <div key={label} className="flex items-center gap-3">
              <div className={`p-1.5 rounded-lg shrink-0 ${darkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-600'}`}>
                <Icon size={12} />
              </div>
              <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
                <span className={`text-[10px] font-medium ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>{label}</span>
                <span className={`text-[11px] font-semibold text-right truncate max-w-[65%] ${darkMode ? 'text-white' : 'text-zinc-800'}`}>{value}</span>
              </div>
            </div>
          ) : null
        )}
      ...
      {booking.travelers?.length > 0 && (
        <div className={`mx-5 mb-3.5 p-3 rounded-2xl border ${darkMode ? 'bg-zinc-800/50 border-white/5' : 'bg-zinc-50/80 border-zinc-200/70'}`}>
          <p className={`text-[9px] font-bold uppercase tracking-wider mb-2 ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
            Registered Hikers ({booking.travelers.length})
          </p>
          {booking.travelers.map((t, i) => (
            <div key={i} className={`flex items-center justify-between py-1 border-b last:border-0 ${darkMode ? 'border-zinc-800/50' : 'border-zinc-200/60'}`}>
              <span className={`text-xs font-semibold ${darkMode ? 'text-white' : 'text-zinc-800'}`}>{t.name}</span>
              <span className={`text-[11px] ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>Age {t.age} · {t.gender}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {booking.organizerName && (
        <div className="mx-5 mb-4 flex items-center gap-2 p-2.5 rounded-xl bg-spy-orange/10 border border-spy-orange/20">
          ...
          <p className="text-xs font-semibold text-spy-orange">{booking.organizerName}</p>
        </div>
      )}
    </motion.div>
  );
...
export default function OrgScannerView({ organizer, trips = [], onBack, darkMode }) {
  ...
  const isMyBooking = useCallback((b) => {
    if (!b) return false;
    const orgEmail = (organizer?.email || '').toLowerCase().trim();
    const orgAgency = (organizer?.agencyName || organizer?.name || '').toLowerCase().trim();
    ...
    if (bTripId && Array.isArray(trips) && trips.some(t => String(t._id || t.id) === bTripId)) {
      return true;
    }
    return false;
  }, [organizer, trips]);
  ...
  const handleCheckIn = useCallback(async (rawCode) => {
    ...
    try {
      ...
      const isForbidden = status === 403 || /another organization|belongs to another/i.test(err?.message || '');

      if (isForbidden) {
        setErrMsg(err?.message || 'This ticket belongs to another organization. You can only scan and verify tickets for your own treks.');
        setPhase('error');
        setChecking(false);
        return;
      }
        ...
        if (storedBookingsRaw) {
          ...
          if (matched) {
            if (!isMyBooking(matched)) {
              const otherOrg = matched.organizerName || 'another organization';
              setErrMsg(`This ticket belongs to another organization (${otherOrg}). You can only scan and verify tickets for your own treks.`);
              setPhase('error');
              setChecking(false);
              return;
            }
            setBooking(matched);
            setAlreadyCheckedIn(matched.status === 'Completed' || !!matched.checkedInAt);
            setPhase('found');
            setChecking(false);
            return;
          }
  ...
  return (
    <div className={`w-full max-h-[85vh] flex flex-col rounded-3xl overflow-hidden shadow-2xl border ${
      darkMode ? 'bg-zinc-950 border-white/10 text-white' : 'bg-[#FAF8F2] border-zinc-200/90 text-zinc-900'
    }`}>

      {/* Header */}
      <div className={`flex items-center justify-between px-5 sm:px-6 pt-4 sm:pt-5 pb-3 shrink-0 border-b ${
        darkMode ? 'bg-zinc-900/80 border-white/5' : 'bg-white/80 backdrop-blur-md border-zinc-200/80 shadow-xs'
      }`}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-base font-display font-black tracking-tight">Hiker Ticket Scanner</h1>
            <p className={`text-[10px] ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>Scan QR barcode or enter booking reference</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-4 min-h-0 space-y-3">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="err" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
              className={`rounded-3xl p-6 border text-center ${
                /another organization|unauthorized/i.test(errMsg)
                  ? darkMode ? 'bg-amber-950/25 border-amber-500/40 text-white' : 'bg-amber-50/90 border-amber-200 text-zinc-900 shadow-sm'
                  : darkMode ? 'bg-zinc-800/60 border-zinc-700 text-white' : 'bg-white border-zinc-200/80 text-zinc-900 shadow-sm'
              }`}>
              {/another organization|unauthorized/i.test(errMsg) ? (
              ...
              <h3 className="font-display font-black text-base">
                {/another organization|unauthorized/i.test(errMsg) ? 'Unauthorized Ticket' : 'Booking Not Found'}
              </h3>
              <p className={`text-xs mt-2 max-w-sm mx-auto leading-relaxed ${
                /another organization|unauthorized/i.test(errMsg)
                  ? darkMode ? 'text-amber-200/80' : 'text-amber-900'
          ...
          {/* Found */}
          {phase === 'found' && booking && (
            <motion.div key="found" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3 pb-2">
      ...
      {phase === 'found' && (
        <div className={`shrink-0 p-4 border-t ${
          darkMode ? 'bg-zinc-900/90 border-white/5' : 'bg-white/90 backdrop-blur-md border-zinc-200/80'
        }`}>
          <button
            type="button"
