...
const LEGAL_SECTIONS = [
  {
    ...
    body: 'By listing treks on Spy Hike, organizers agree to maintain accurate trip information, honor published pricing and cancellation policies, and hold valid guiding/operating licenses for every region they operate in.',
  },
  {
    icon: ShieldCheck,
    title: 'Privacy Policy',
    body: 'Traveller contact details shared for an upcoming booking may only be used to coordinate that trek. Spy Hike encrypts all verification documents and never shares them with third parties without consent.',
  },
  {
    icon: HandHeart,
    title: 'Partner Agreement',
    body: 'Spy Hike charges a platform commission per completed booking (visible on every payout breakdown) in exchange for traveller discovery, secure payments, and dispute resolution support.',
  },
];
    ...
    <div className={`h-full flex flex-col overflow-hidden font-sans ${darkMode ? 'bg-zinc-950 text-white' : 'bg-gray-50 text-zinc-800'}`}>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-5 space-y-5 pb-10">
        ...
        <div className="flex flex-col items-center text-center pt-4 pb-2">
          <div className="w-16 h-16 bg-spy-orange rounded-2xl flex items-center justify-center shadow-lg shadow-spy-orange/20 transform rotate-3">
            <Compass className="w-9 h-9 text-white -rotate-3" />
          </div>
          <h1 className="text-2xl font-display font-black tracking-tight mt-4">Spy Hike</h1>
        ...
        <div className={`p-4 rounded-2xl ${cardCls}`}>
          <p className={`text-xs leading-relaxed ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
            Spy Hike connects certified trek operators with adventure travellers across India's Himalayan and Western Ghats trails.
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            <a href="https://spyhike.com" className={`flex flex-col items-center gap-1.5 py-3.5 rounded-2xl transition active:scale-95 ${cardCls}`}>
        ...
        <p className={`text-center text-[11px] pt-3 ${darkMode ? 'text-zinc-600' : 'text-zinc-400'}`}>
          Made with ❤️ for mountain explorers everywhere.<br />© 2026 Spy Hike. All rights reserved.
        </p>
      </div>
    </div>
  );
