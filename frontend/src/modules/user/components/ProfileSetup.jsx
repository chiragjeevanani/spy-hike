...
export default function ProfileSetup({ user, onComplete, darkMode }) {
  ...
  return (
    <div className={`h-full flex flex-col overflow-y-auto font-sans px-5 py-6 ${
      darkMode ? 'bg-zinc-950 text-white' : 'bg-gray-50 text-zinc-800'
    } no-scrollbar`}>
      
      {/* Upper header segment */}
      <div className="flex flex-col items-center shrink-0 pt-2 pb-5">
        <div className="p-3.5 rounded-2xl flex justify-center items-center shadow-lg bg-forest-600/10 text-forest-600 dark:text-forest-400">
          <Compass className="w-8 h-8 animate-spin-slow" />
        </div>
        <h1 className="font-display font-black tracking-tight text-xl mt-3 text-forest-600 dark:text-forest-400">
          Set Up Your Hiker Profile
        </h1>
        <p className={`text-xs mt-1.5 text-center px-4 max-w-sm leading-relaxed ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
          Provide experience & safety details to enable alpine route approvals.
        </p>
      </div>

      <div className={`w-full rounded-3xl shadow-xl p-6 mb-6 ${
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div id="setup-experience" ref={el => { fieldRefs.current.experience = { current: el }; }} className="space-y-2">
            <label className="text-[11px] font-bold uppercase tracking-wider opacity-85 flex items-center gap-1.5">
              <Award size={13} className="text-forest-500" /> Hiking Experience Level *
            </label>
            <div className={`grid grid-cols-3 gap-2 rounded-xl ${fieldErrors.experience ? 'ring-2 ring-red-500' : ''}`}>
              {['Beginner', 'Intermediate', 'Advanced'].map((lvl) => (
                <button
                  key={lvl}
                  type="button"
                  onClick={() => { setHikingExperience(lvl); setFieldErrors(er => ({ ...er, experience: '' })); }}
                  className={`py-3 rounded-xl border font-semibold text-[11px] transition-all cursor-pointer ${
          ...
          <div id="setup-fitness" ref={el => { fieldRefs.current.fitness = { current: el }; }} className="space-y-2">
            <label className="text-[11px] font-bold uppercase tracking-wider opacity-85 flex items-center gap-1.5">
              <HeartPulse size={13} className="text-spy-orange" /> Fitness Conditioning *
            </label>
            <div className={`grid grid-cols-3 gap-2 rounded-xl ${fieldErrors.fitness ? 'ring-2 ring-red-500' : ''}`}>
          ...
          <div id="setup-gender" ref={el => { fieldRefs.current.gender = { current: el }; }} className="space-y-2">
            <label className="text-[11px] font-bold uppercase tracking-wider opacity-85 flex items-center gap-1.5">
              <User size={13} className="text-blue-500" /> Gender *
            </label>
            <div className={`grid grid-cols-3 gap-2 rounded-xl ${fieldErrors.gender ? 'ring-2 ring-red-500' : ''}`}>
          ...
          <div id="setup-age" className="space-y-1.5">
            <label className="text-[11px] font-bold uppercase tracking-wider opacity-85 block">Age *</label>
            <input
          ...
          {/* 5. Emergency Contact */}
          <div id="setup-emergency" className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5 min-w-0">
              <label className="text-[11px] font-bold uppercase tracking-wider opacity-85 block">
          ...
          <div className="space-y-3">
            <label className="text-[11px] font-bold uppercase tracking-wider opacity-85 flex items-center gap-1.5">
              <Sparkles size={13} className="text-amber-500" /> Choose Hiker Avatar (Optional)
            </label>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-forest-600 hover:bg-forest-700 text-white font-bold py-3.5 rounded-xl shadow-lg mt-3 cursor-pointer active:scale-98 transition-all disabled:opacity-60 disabled:cursor-not-allowed text-xs uppercase tracking-wider"
          >
