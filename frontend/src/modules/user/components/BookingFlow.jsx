...
import { 
  ...
  Sparkles, Percent, ShieldCheck, Download, Share2, Info, Landmark, X
} from 'lucide-react';
...
export default function BookingFlow({
  ...
  const handleValidateCoupon = (e) => {
    ...
    if (formatted === 'SPYHIKE20') {
      ...
      setDiscountPercent(15); // simulate % off for simple Math calculation
    ...
    <div className={`flex-1 flex flex-col justify-between overflow-y-auto no-scrollbar font-sans px-6 py-4 relative ${
      ...
      {step < 6 && (
        <div className="shrink-0 flex items-center justify-between pb-4 border-b border-zinc-800/10 dark:border-zinc-850">
          ...
          <div className="text-center">
            ...
            <h3 className="text-sm font-display font-black text-forest-650 dark:text-forest-400">Step {step} of 5</h3>
          </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-display font-black">Select Expedition Date</h2>
            </div>
            <p className="text-xs text-zinc-500">
              Verified guides execute strict roster departures. Pick your ideal mountaineering slot:
            </p>
                ...
                return (
                  <label
                    ...
                    className={`p-3 rounded-xl flex items-center justify-between cursor-pointer transition-all border ${
                      isSelected
                        ? darkMode
                          ? 'bg-forest-950/30 border-forest-500 shadow-md shadow-forest-950/20 text-white'
                          : 'bg-forest-50/70 border-forest-500 shadow-sm text-forest-900 font-bold'
                        : darkMode
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {/* Mini calendar box */}
                      <div className={`w-9 h-9 rounded-lg flex flex-col items-center justify-center text-center shrink-0 ${
                        isSelected
                          ? darkMode ? 'bg-forest-600/30 text-forest-400 border border-forest-500/20' : 'bg-forest-600 text-white'
                          : darkMode ? 'bg-zinc-800 text-zinc-400 border border-zinc-700/50' : 'bg-gray-100 text-zinc-500 border border-zinc-200'
                      }`}>
                        <span className="text-[7.5px] uppercase font-black tracking-wider leading-none">{monthName}</span>
                        <span className="text-[13px] font-black leading-none mt-0.5">{dayNum}</span>
                      </div>

                      <div className="flex flex-col">
                        <span className="text-[11.5px] font-bold font-sans">
                          {dayName}, {dayNum} {monthName} {yearNum}
                        </span>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span className={`text-[8px] font-black tracking-wider px-1.5 py-0.2 rounded-full uppercase ${
                            isSelected
                    ...
                    <input
                      ...
                      className="accent-forest-500 pointer-events-auto w-4 h-4 cursor-pointer"
                    />
                  </label>
                );
              })}
            ...
            <div className={`p-3 rounded-xl flex gap-2.5 ${darkMode ? 'bg-zinc-900/20' : 'bg-white shadow-xs'}`}>
              <Info size={16} className="text-spy-orange shrink-0 mt-0.5" />
              <p className="text-[10px] leading-relaxed text-zinc-400">
                Departures are subject to safe dynamic weather clearance. In case of storm flags, custom slots roll over automatically at zero fee penalty.
              </p>
            </div>
          </div>
        )}

        {/* Step 2: Select Number Of Travelers */}
        {step === 2 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <Users className="text-forest-500" size={18} />
              <h2 className="text-base font-display font-black">Specify Travelers Count</h2>
            </div>
            <p className="text-xs text-zinc-500">
              Maximum roster constraints allow {trip.maxGroupSize} participants. How many seats would you like to guard?
            </p>

            {/* Traveler selector counters */}
            <div className="flex items-center justify-center gap-6 py-8">
              <button
                type="button"
                disabled={travelersCount <= 1}
                onClick={() => setTravelersCount(prev => prev - 1)}
                className={`w-12 h-12 rounded-full border flex items-center justify-center text-xl font-bold cursor-pointer transition ${
                  travelersCount <= 1 
              ...
              <span className="text-4xl font-display font-black font-mono">{travelersCount}</span>
            ...
            <div className={`p-4 rounded-xl text-center ${
              darkMode ? 'bg-zinc-900' : 'bg-white shadow-xs'
            }`}>
              <span className="text-[10px] uppercase font-bold tracking-widest opacity-50 block">ESTIMATED TOUR BASE COST</span>
              <span className="text-2xl font-black font-sans mt-1 text-forest-650 dark:text-forest-400 block">₹{baseCostTotal}</span>
              <span className="text-[9px] opacity-40 mt-1 block">(₹{trip.price} x {travelersCount} Travelers)</span>
            </div>
          </div>
        )}

        {/* Step 3: Add Traveler Details */}
        {step === 3 && (
          <div className="space-y-4 max-h-[460px] overflow-y-auto no-scrollbar pointer-events-auto pr-1">
            ...
            <p className="text-xs text-zinc-500 pb-2">
              Required for emergency park transit passes, rescue permissions and state environmental insurance registry:
            </p>
              ...
              {travelersList.map((tr, idx) => (
                <div 
                  ...
                  className={`p-4 rounded-2xl space-y-3 relative ${
                    darkMode ? 'bg-zinc-900/60' : 'bg-white shadow-sm'
                  }`}
                >
        ...
        {step === 4 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <Ticket className="text-forest-500" size={18} />
              <h2 className="text-base font-display font-black">Redeem Promo Coupons</h2>
            </div>
            <p className="text-xs text-zinc-500">
              Apply active vouchers to decrease environmental pass fee calculations:
            </p>

            <form onSubmit={handleValidateCoupon} className="flex gap-2 pt-2">
              <input
                type="text"
                placeholder="PROMO CODE e.g. SPYHIKE20"
                value={couponCode}
                onChange={e => setCouponCode(e.target.value)}
                className={`flex-1 text-xs px-3 py-3 border rounded-xl outline-hidden focus:border-forest-500 uppercase tracking-widest ${
                  darkMode ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-gray-255 text-zinc-850'
                }`}
              />
              <button
                type="submit"
                id="btn-apply-coupon"
                className="bg-forest-600 hover:bg-forest-700 text-white text-xs font-bold px-4 rounded-xl cursor-pointer"
              >
                Apply Coupon
              </button>
            </form>
            ...
            {couponError && (
              <span className="text-[11px] font-bold text-rose-500 block">{couponError}</span>
            )}
            {couponSuccess && (
              <span className="text-[11px] font-bold text-emerald-400 block">{couponSuccess}</span>
            )}

            {/* Quick Suggestions Cards */}
            <div className="space-y-2 pt-4">
              <span className="text-[10px] font-bold uppercase tracking-wider opacity-60">ACTIVE CAMPAIGN DEALS</span>
              
              <div 
                onClick={() => { setCouponCode('SPYHIKE20'); }}
                className={`p-3 rounded-xl border border-dashed flex items-center justify-between cursor-pointer transition ${
                  darkMode ? 'bg-zinc-900/40 border-forest-800/40 hover:bg-zinc-900' : 'bg-white border-gray-300'
                }`}
              >
                <div>
                  <h4 className="text-xs font-bold text-forest-500 flex items-center gap-1">
                    <Percent size={11} /> SPYHIKE20
                  </h4>
                  <p className="text-[9px] text-zinc-500 block">Save flat 20% on any trekking crossings</p>
                </div>
                <span className="text-[9px] uppercase tracking-wider text-rose-400 font-black">USE DEAL</span>
              </div>
            </div>
          </div>
        )}

        {/* Step 5: Payment Screen Checkout */}
        {step === 5 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <CreditCard className="text-forest-500" size={18} />
              <h2 className="text-base font-display font-black">Payment & Pass Settlement</h2>
            </div>
            ...
            <div className={`p-4 rounded-2xl space-y-3 border ${
              ...
              <div className="flex justify-between text-xs">
                <span className="opacity-70">National Park Taxes (5% GST)</span>
            ...
            <div className="flex items-center justify-center gap-1.5 pt-6 text-[10.5px] opacity-75 font-semibold text-zinc-500">
              <ShieldCheck size={13} className="text-forest-660 dark:text-forest-400" />
              <span>Payments secured and processed via Razorpay Express</span>
            </div>

            {isProcessingPayment && (
              <div className="p-4 rounded-xl bg-orange-500/10 border border-orange-500/35 flex items-center justify-center gap-3">
                <div className="w-4 h-4 rounded-full border-2 border-spy-orange border-t-transparent animate-spin" />
            ...
            <div className={`p-4 rounded-3xl border border-dashed relative overflow-hidden text-left mx-auto max-w-sm ${
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1.5 text-[9px] text-zinc-400">
                  ...
                  <span>•</span>
                  <span>👨 {createdBooking.travelersCount} Hiker</span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <span className={`text-xs font-extrabold font-sans ${darkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>₹{createdBooking.finalAmount}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-center gap-3 pt-6 cursor-pointer pointer-events-auto">
              <button
                ...
                className={`px-4 py-3 rounded-xl border text-xs font-extrabold flex items-center justify-center gap-1.5 transition ${
                  darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-350 hover:bg-zinc-850' : 'bg-white border-gray-250 text-zinc-700'
                }`}
              >
      ...
      {step < 5 && (
        <div className="pt-6 border-t border-zinc-800/10 dark:border-zinc-850 flex justify-end shrink-0 gap-3">
          ...
          <button
            ...
            onClick={() => setStep(prev => prev + 1)}
            className="flex-1 bg-forest-600 hover:bg-forest-700 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-1.5 text-xs shadow-md"
          >
            Continue
            <ArrowRight size={14} />
          </button>
      ...
      {step === 5 && (
        <div className="pt-6 border-t border-zinc-800/10 dark:border-zinc-850 shrink-0">
          <button
            ...
            className={`w-full bg-forest-600 hover:bg-forest-700 text-white font-black py-4 rounded-xl flex items-center justify-center gap-1.5 text-xs shadow-md ${
              isProcessingPayment ? 'opacity-50 cursor-not-allowed' : 'active:scale-98 cursor-pointer'
            }`}
          >
            Proceed and Pay ₹{finalPayAmount} <ShieldCheck size={14} />
          </button>
        </div>
      )}
          ...
          <div className={`p-6 rounded-3xl max-w-xs text-center relative ${
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] font-bold">
              <button 
                ...
                className="py-2.5 rounded-lg bg-green-650 hover:bg-green-750 text-white"
              >
                WhatsApp Invite
              </button>
              <button 
        ...
        <div className="fixed inset-0 bg-black/80 z-55 flex items-center justify-center p-6">
          <div className={`p-6 rounded-3xl max-w-sm text-center relative w-full ${
            ...
            <h4 className="text-sm font-bold font-display flex items-center gap-1 text-forest-600 dark:text-forest-400 text-center justify-center mb-1">
              <CheckCircle2 size={15} /> Offical Mountain Pass
            </h4>
            ...
            <div className="space-y-4 text-left my-4 p-4 rounded-xl bg-zinc-950 shadow-inner font-mono border border-zinc-850">
              <div className="flex justify-between text-[10px] pb-2 border-b border-zinc-850">
                ...
                <span className="text-orange-400 font-bold">{createdBooking?.bookingId}</span>
              </div>
              <div className="flex justify-between text-[10px] pb-2 border-b border-zinc-850">
              ...
              <div className="flex justify-between text-[10px]">
                ...
                <span className={darkMode ? 'text-emerald-400' : 'text-emerald-700'}>₹{createdBooking?.finalAmount}</span>
              </div>
            </div>
