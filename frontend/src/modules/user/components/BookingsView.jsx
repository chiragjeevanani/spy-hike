...
export default function BookingsView({
  ...
  const currentChatSession = activeChatSession
    ? chats.find((c) => c.tripId === activeChatSession.tripId) ||
      activeChatSession
    : null;
  ...
  const handleContactOrganizer = (booking) => {
    ...
    if (existing) {
      const updated =
        !existing.organizerAvatar ||
        existing.organizerAvatar.includes("photo-1534528741775-53994a69daeb")
  ...
  const simulateOrganizerReply = (tripId, baseSession) => {
    setTimeout(() => {
      ...
      const finalSession = {
        ...baseSession,
        messages: [...baseSession.messages, botMsg],
      };
  ...
  const handleSubmitReview = (e) => {
    ...
    if (!reviewComment.trim()) {
      ...
      reviewCommentRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    ...
    toast.success(
      "Thank you! Your verified hiking rating has been registered successfully.",
    );
  ...
  return (
    <div
      className={`flex-1 font-sans w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-28 md:pb-16 ${
        darkMode
      ...
      <div className="pt-6">
        ...
        <p
          className={`text-sm mt-1 ${darkMode ? "text-zinc-400" : "text-zinc-500"}`}>
          Your treks, past and upcoming
        </p>

        {/* Segmented tabs */}
        <div
          className={`flex rounded-full p-1 mt-5 relative max-w-md ${darkMode ? "bg-elegant-card" : "bg-gray-100"}`}>
          {["Upcoming", "Completed", "Cancelled"].map((tab) => (
            <button
              ...
              <span
                className={`relative z-20 transition-colors ${activeTab === tab ? "text-white" : darkMode ? "text-zinc-400" : "text-zinc-500"}`}>
                {tab}
              </span>
              {activeTab === tab && (
                <motion.div
      ...
      <div className="mt-8">
        {filteredBookings.length === 0 ? (
          <div className="text-center py-20">
            <CalendarDays
              size={44}
              className={`mx-auto ${darkMode ? "text-zinc-700" : "text-zinc-300"}`}
            />
            <h3 className="font-serif text-2xl font-semibold mt-4">
              Nothing here yet
            </h3>
            <p
              className={`text-sm mt-2 max-w-xs mx-auto leading-relaxed ${darkMode ? "text-zinc-500" : "text-zinc-400"}`}>
              No {activeTab.toLowerCase()} treks. Head to Explore to find your
              next adventure.
            </p>
          </div>
        ) : (
            ...
            {filteredBookings.map((b, idx) => (
              <motion.div
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                  <div>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <span className="text-[10px] font-mono opacity-50 uppercase tracking-wider block">
                          #{b.bookingId.slice(-8).toUpperCase()}
                        </span>
                        <h3 className="font-serif text-sm sm:text-base lg:text-lg font-semibold leading-snug break-words line-clamp-2 pr-1 mt-0.5">
                          {b.tripName}
                        </h3>
                      </div>
                      {(() => {
                        ...
                        return (
                          <span
                            className={`text-[9px] sm:text-[10px] font-bold tracking-wider px-2.5 py-1 rounded-full shrink-0 border whitespace-nowrap self-start ${badge.cls}`}>
                    ...
                    <p
                      className={`text-xs truncate mt-1 flex items-center gap-1 ${darkMode ? "text-zinc-400" : "text-zinc-500"}`}>
                  ...
                  <div
                    className={`flex items-center justify-between mt-3 pt-2.5 border-t gap-2 flex-wrap sm:flex-nowrap ${darkMode ? "border-white/5" : "border-gray-100"}`}>
                    <span
                      className={`text-xs font-medium flex items-center gap-1 whitespace-nowrap ${darkMode ? "text-zinc-400" : "text-zinc-500"}`}>
                      <CalendarDays
                        size={13}
                    ...
                    <div className="flex items-center gap-2.5 shrink-0 ml-auto">
                      <button
                        ...
                        className={`w-8 h-8 rounded-full flex items-center justify-center cursor-pointer active:scale-90 transition ${
                          darkMode
                            ? "bg-white/5 text-forest-400 hover:bg-white/10"
                            : "bg-forest-50 text-forest-600 hover:bg-forest-100"
                        }`}>
                        <MessageSquare size={14} />
                      </button>
                      <span
                        className={`font-serif text-base sm:text-lg font-bold whitespace-nowrap ${darkMode ? "text-elegant-text" : "text-zinc-900"}`}>
                        ₹{b.finalAmount}
                      </span>
                    </div>
                  </div>
        ...
        {currentChatSession &&
          (() => {
            const matchedBooking = bookings.find(
              (b) => b.tripId === currentChatSession.tripId,
            );
            const matchedTrip = trips?.find(
              (t) => t.id === currentChatSession.tripId,
            );
            const tripName =
              currentChatSession.tripName ||
              matchedBooking?.tripName ||
              matchedTrip?.name;
            const departureDate = matchedBooking?.selectedDate;
            const avatarUrl =
              currentChatSession.organizerAvatar &&
              !currentChatSession.organizerAvatar.includes(
            ...
            return (
              <div
                ...
                <motion.div
                  ...
                  <div
                    className={`p-4 sm:px-5 border-b flex flex-col gap-2 shrink-0 ${
                      darkMode
                        ? "bg-zinc-900/60 border-white/10"
                        : "bg-gray-50/80 border-zinc-200/80"
                    }`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-emerald-500 dark:text-emerald-400 font-semibold flex items-center gap-1 mt-0.5">
                            <Sparkles size={9} /> Online Guide Counselor · Ready
                            to help
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className={`w-8 h-8 rounded-full flex items-center justify-center transition active:scale-95 cursor-pointer ${
                          darkMode
                            ? "bg-zinc-800/80 hover:bg-zinc-700 text-zinc-350"
                            : "bg-gray-200/70 hover:bg-gray-300 text-zinc-600"
                        }`}>
                        <X size={16} />
                      </button>
                    </div>
                    ...
                    {tripName && (
                      <div
                        className={`px-2.5 py-1 rounded-xl text-[10px] font-medium flex items-center justify-between border ${
                          darkMode
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-4 sm:p-5 space-y-3.5">
                    <div className="flex justify-center">
                      <span
                        className={`text-[9px] font-mono font-semibold px-2.5 py-0.5 rounded-full border ${
                          darkMode
                            ? "bg-zinc-900 border-white/5 text-zinc-500"
                            : "bg-gray-100 border-zinc-200 text-zinc-400"
                        }`}>
                    ...
                    {currentChatSession.messages.map((m) => {
                      ...
                      return (
                        <div
                          ...
                          <div
                            className={`p-3.5 max-w-[82%] rounded-2xl text-xs leading-relaxed relative shadow-xs ${
                              isUser
                            ...
                            <span
                              className={`text-[8px] font-mono self-end block mt-1.5 text-right ${
                  ...
                  {/* Inputs bar footer */}
                  <div
                    className={`p-3.5 sm:p-4 border-t shrink-0 pb-[max(0.875rem,env(safe-area-inset-bottom))] ${
                      darkMode
                      ...
                      <input
                        ...
                        className={`flex-1 text-xs px-4 py-3 border rounded-xl outline-hidden focus:border-forest-500 transition ${
                          darkMode
                            ...
                            : "bg-white border-gray-250 text-zinc-800 placeholder-zinc-400"
                        }`}
                      />
                      <button
                        type="button"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className={`w-full text-xs p-3 border rounded-xl outline-hidden focus:border-forest-500 ${
                    darkMode
                      ? "bg-zinc-950 border-zinc-855 text-white"
                      : "bg-gray-100 border-gray-255 text-zinc-900"
          ...
          <div className="fixed inset-0 bg-black/75 z-55 flex items-center justify-center p-6">
            <motion.div
              ...
              <div className="my-4 space-y-3 text-xs">
                ...
                <div
                  className={`flex justify-between font-bold pt-1 text-sm ${darkMode ? "text-emerald-400" : "text-emerald-700"}`}>
                  <span>Final amount settled:</span>
                  <span className="font-sans">
                    ₹{showInvoiceBooking.finalAmount}
                  </span>
                </div>
              </div>
              ...
              <button
                onClick={() => {
                  toast.success("Invoice saved successfully to directory.");
