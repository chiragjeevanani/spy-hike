...
async function makeTrip(orgToken) {
  const res = await request(app).post('/api/v1/organizer/trips').set('Authorization', `Bearer ${orgToken}`).send({
    name: 'Social Trek', location: 'Manali',
...
describe('Reviews', () => {
  ...
  it('rejects a second review of the same booking (409)', async () => {
    ...
    const trip = await makeTrip(org);
    const booking = await makeBooking(token, trip.id);
    await request(app).post(`/api/v1/bookings/${booking.bookingId}/review`).set('Authorization', `Bearer ${token}`).send({ rating: 4 });
    const dup = await request(app).post(`/api/v1/bookings/${booking.bookingId}/review`).set('Authorization', `Bearer ${token}`).send({ rating: 3 });
